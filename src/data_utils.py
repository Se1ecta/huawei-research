...
def prepare_multimodal_messages(
    messages: list[dict[str, Any]], images: list | None = None
) -> list[dict[str, Any]]:
    ...
    for message in messages:
        if isinstance(message["content"], list):
            for part in message["content"]:
                if part["type"] == "image":
                    part["type"] = (
                        "image_pil"  # vLLM expects 'image_pil' key for images
                    )
...
def apply_chat_template(
    ...
    **template_kwargs,
    ...
    tools = (
        tools or None
    )  # `or None`: Llama bug: it renders tool boilerplate for tools=[]
    ...
    if "prompt" in example:  # explicit prompt and prompt-completion case
        ...
        if "rejected" in example and "prompt" in example:  # explicit prompt
            ...
            prompt = "".join(
                x
                for x, _ in takewhile(
                    lambda x: x[0] == x[1], zip(prompt, prompt_rejected, strict=False)
                )
            )
        ...
        if "completion" in example:
            ...
            prompt = "".join(
                x
                for x, _ in takewhile(
                    lambda x: x[0] == x[1], zip(prompt, prompt_completion, strict=False)
                )
            )
...
def _get_dataset_format(dataset: DatasetType) -> dict[str, Any]:
    ...
    if isinstance(dataset, Dataset):
        ...
        format_type = (
            dataset._formatting.format_type if dataset._formatting is not None else None
        )
...
def _check_if_columns_can_be_packed(columns: list[pa.Array]):
    ...
    for idx, column in enumerate(columns):
        if not (
            pyarrow.types.is_list(column.type)
            or pyarrow.types.is_large_list(column.type)
        ):
...
def _pack_bfd(
    examples: pa.Table,
    seq_length: int,
    on_seq_length_overflow: Literal["truncate", "split"] = "truncate",
) -> pa.Table:
    ...
    if on_seq_length_overflow == "truncate":
        ...
        offsets = (
            np.arange(
                np.sum(num_fragments) + 1,
                dtype=columns[0].offsets.type.to_pandas_dtype(),
            )
            * seq_length
        )
        ...
        columns = [
            type(column).from_arrays(
                offsets.astype(column.offsets.type.to_pandas_dtype()), column.values
            )
            for column in columns
        ]
    else:
        raise ValueError(
            f"Invalid `on_seq_length_overflow`: {on_seq_length_overflow}. Use 'truncate' or 'split'."
        )
    ...
    bins: list[dict] = []
    for length, idx in zip(
        lengths.field(0).to_numpy(), lengths.field(1).to_numpy(), strict=True
    ):
...
def _pack_wrapped(examples: pa.Table, seq_length: int) -> pa.Table:
    ...
    offsets = np.arange(
        0, num_elements, seq_length, dtype=columns[0].offsets.type.to_pandas_dtype()
    )
...
def pack_dataset(
    ...
    if strategy not in valid_strategies:
        raise ValueError(
            f"Invalid packing strategy '{strategy}', must be one of {valid_strategies}."
        )
...
def is_conversational_from_value(example: dict[str, Any]) -> bool:
    ...
    if isinstance(maybe_messages, list):
        ...
        if (
            ...
            and "from" in maybe_message
            and "value" in maybe_message
        ):
            return True
