...
Bias-corrected moment estimates are then computed:
...
MeZO (Memory-efficient Zeroth-Order Optimization)~\cite{malladi2023mezo} is a gradient-free optimization method that estimates directional derivatives using forward passes only, eliminating the need for backpropagation.

At each step, a random perturbation direction $z \sim \mathcal{N}(0, I)$ is sampled, and the directional derivative is approximated via finite differences:

\begin{equation}
\hat{g}_t \approx \frac{\mathcal{L}(\theta_t + \epsilon z) - \mathcal{L}(\theta_t - \epsilon z)}{2\epsilon} \cdot z
\end{equation}

where $\epsilon$ is a small perturbation scalar and $\mathcal{L}$ denotes the loss function. Parameters are then updated as:

\begin{equation}
\theta_{t+1} = \theta_t - \eta \hat{g}_t
\end{equation}

This approach substantially reduces memory consumption by avoiding storage of gradients and intermediate activations. However, it introduces variance in gradient estimation and requires two forward passes per update, doubling computational cost per step.
...
\begin{quote}
\textit{``We believe this optimizer is unlikely to work well for training with small batch size. 
...
Overall, downstream differences between AdamW, Muon and Hybrid (AdamW+Muon) remain negligible, while MeZO exhibits clear degradation. The marginal improvements over the base model suggest that short-horizon full fine-tuning on a 10k subset provides limited task-level benefit in this regime.

The absence of downstream improvements suggests that short-horizon full fine-tuning on a small generic corpus may not enhance zero-shot reasoning performance. This behavior may reflect mild catastrophic forgetting or insufficient dataset scale to induce beneficial adaptation. Consequently, the downstream evaluation primarily reflects differences in optimization stability rather than meaningful task-level gains.
